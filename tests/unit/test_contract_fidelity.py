...
CONTRACT_CASES = [
    ...
    ("PostureDimensions nested",
     lambda: ProjectPostureResponse(
         posture=72.5, grade="C",
         dimensions={
             "security": {"posture": 65.0, "grade": "C",
    ...
    ("Findings page (one)",
     lambda: FindingsResponse(
         data=[{
             "id": "find-1", "project_id": "proj-456",
             "title": "Prompt injection risk", "description": "...",
             "category": "adversarial", "threat_class": "prompt_injection",
             "severity": 75.0, "severity_label": "high", "confidence": 0.9,
             "status": "open", "occurrence_count": 3,
             "delegation_status": "unassigned",
         }],
         total=1, page=1, size=50, has_next_page=False,
     ).model_dump(),
     FindingsResponse),

    ("Logs page",
     lambda: PaginationResponseLogs(
         data=[{"id": "log-1", "result": "fail", "prompt": "...",
                "severity": "high", "fail_category": "prompt_injection"}],
         total=1, page=1, size=50, has_next_page=False,
     ).model_dump(),
     PaginationResponseLogs),
    ...
    ("Experiments page",
     lambda: PaginationResponseExperiments(
         data=[{"id": "exp-abc", "name": "quick", "status": "Finished",
                "test_category": "owasp_agentic", "testing_level": "unit"}],
         total=1, page=1, size=50, has_next_page=False,
     ).model_dump(),
     PaginationResponseExperiments),
    ...
    ("Projects page",
     lambda: PaginationResponseProjects(
         data=[{"id": "proj-456", "name": "Demo", "description": "",
                "archived": False}],
         total=1, page=1, size=50, has_next_page=False,
     ).model_dump(),
     PaginationResponseProjects),
    ...
    ("Experiment single",
     lambda: ExperimentsResponse(
         id="exp-abc", name="quick", status="Finished",
         test_category="owasp_agentic", testing_level="unit",
     ).model_dump(),
     ExperimentsResponse),

    ("Experiment status",
     lambda: ExperimentsResponseStatus(id="exp-abc", status="Running",
                                        progress=0.42).model_dump(),
     ExperimentsResponseStatus),

    ("Project single",
     lambda: ProjectsResponse(id="proj-456", name="Demo",
                               description="").model_dump(),
     ProjectsResponse),

    ("Provider single",
     lambda: ProvidersResponse(id="prov-1", name="openai",
                                integration={"api_key": "sk-..."}).model_dump(),
     ProvidersResponse),
    ...
    ("ApiKey single",
     lambda: ApiKeyResponse(id="key-1", name="CI",
                             key_prefix="hbk_abc12", status="active").model_dump(),
     ApiKeyResponse),

    ("ApiKeys page",
     lambda: PaginationResponseApiKeys(
         data=[{"id": "key-1", "name": "CI", "key_prefix": "hbk_abc12",
                "status": "active"}],
         total=1, has_next_page=False,
     ).model_dump(),
     PaginationResponseApiKeys),

    ("OK envelope",
     lambda: OKMessage(message="OK").model_dump(),
     OKMessage),
]
